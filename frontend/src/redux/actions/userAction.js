...
export const userSignInAction = (user) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post(`localhost:3000/api/signin`, user);
...
export const userSignUpAction = (user) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post(`localhost:3000/api/signup`, user);
...
export const userLogoutAction = () => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.get(`localhost:3000/api/logout`);
        dispatch({
...
export const userProfileAction = () => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(`localhost:3000/api/me`);
        dispatch({
